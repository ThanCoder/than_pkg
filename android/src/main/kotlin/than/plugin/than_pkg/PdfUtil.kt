...
object PdfUtil {
    fun callCheck(call: MethodCall, result: Result, context: Context, activity: Activity?) {
        ...
        when (method) {
            "getPdfPageData" -> {
                ...
                val with = call.argument<Int>("width") ?: 0
                val height = call.argument<Int>("height") ?: 0
                val page = call.argument<Int>("page") ?: 0
                if (path == null) {
                    result.error("ERROR", "path is null", "pdf path is null")
                    return
                }
                getPdfPageByteArray(path, page, with, height, { data ->
                    {
                        result.success(data)
                    }
                }, { err ->
                    {
                        result.error("ERROR", "getPdfPageData error", err)
                    }
                })
    ...
    fun getPdfPageByteArray(
        path: String,
        page: Int = 0,
        width: Int = 0,
        height: Int = 0,
        onLoaded: (data: ByteArray) -> Unit,
        onError: (Exception) -> Unit,
    ) {
        executorService.execute {
            try {
